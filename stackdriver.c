 ...
 *
 * To be a bit more concrete, suppose s is a value of type struct stack,
 * s.xs = [2, 4, 6, 8, 1, 3, 5, 7, 9, 0], and x.top = 5.  Then s represents
 * the stack
 *
 *   | 1 |
 *   | 8 |
 ...
 *   -----
 *
 * Pushing the value 11 onto the stack changes s.xs to
 * [2, 4, 6, 8, 1, 11, 5, 7, 9, 0] and s.top = 6, whereas popping the top
 * value does change s.xs at all and changes s.top to 4.
 *
 */
struct stack {
...
struct stack *create() {
  struct stack *stack_loc = malloc(sizeof(struct stack));
    return *stack_loc;
}

bool is_empty(struct stack *s) {
  bool empty = false;
  if (sizeof(struct stack (*s))){
     empty = true;
  }
  else{
  }
